...
import spinal.lib.misc.InterruptCtrl._
...
case class Apb3PassThru(inConfig : Apb3Config) extends Component{
/*
 * I suspect the way to bring out a bus is to use a dummy internally and make the tool
  * think this is an actual component.
*/
  val io = new Bundle{
    val input  =  slave (Apb3(inConfig))
    val output =  master(Apb3(inConfig))

    def default = new Area{
      input <> output
  }
...
class Briey(val config: BrieyConfig) extends Component{
  ...
  val io = new Bundle{
    ...
    val i2c = master(I2c())
    val spi = master(SpiMaster(ssWidth = ssWidth))

    //val axi = Axi4(Axi4Config(addressWidth=16, dataWidth=64, idWidth=4))
    // @TBD: How to do this correctly?
    val apb = master(Apb3(Apb3Config(addressWidth=16, dataWidth=32)))
  ...
  val axi = new ClockingArea(axiClockDomain) {
    ...
    */
    ...
    val  i2cCtrlConfig = I2cSlaveMemoryMappedGenerics(
    ...
    val  spiCtrlConfig = SpiMasterCtrlMemoryMappedConfig(
    ...
    val irqCtrl = Apb3InterruptCtrl(interruptCount)
    //val wbPlic = InterruptCtrl(interruptCount)

    //val axiDummy = Axi4(Axi4Config(addressWidth=16, dataWidth=64, idWidth=4))
    val apbDummy = Apb3PassThru(Apb3Config(addressWidth=16, dataWidth=32))
    ...
    val core = new Area{
      ...
      for(plugin <- config.plugins) plugin match{
        ...
        case plugin : CsrPlugin        => {
          plugin.externalInterrupt := irqCtrl.io.pendings(0) // @TBD: FIXME! Change to an OR?
...
*/
    ...
    val apbDecoder = Apb3Decoder(
      ...
      slaves = List(
        ...
        i2cCtrl.io.apb   -> (0x30000, 4 kB),
        spiCtrl.io.apb   -> (0x40000, 4 kB),
        irqCtrl.io.bus   -> (0x50000, 4 kB),
        apbDummy.io.input-> (0x60000, 4 kB)
        //vgaCtrl.io.apb   -> (0x30000, 4 kB)
      )

    )

    irqCtrl.io.inputs <> Cat(i2cCtrl.io.interrupt, spiCtrl.io.interrupt)
  ...
  io.apb            <> axi.apbDummy.io.output

  //io.axi <> axi.axiDummy.io
