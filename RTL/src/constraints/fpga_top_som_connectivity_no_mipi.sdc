create_clock -name {clk_in} -period 16.6666666666667 [get_ports clk_in]
...
create_clock -name {wb_clk} -period 12.5 [get_nets wb_clk]
...
create_clock -name {tx_clk} -period 5 [get_nets {tx_clk_90 tx_clk}]
create_clock -name {byte_clk} -period 20 [get_nets byte_clk]
set_clock_groups -group [get_clocks clk_in] -group [get_clocks hf_clk] -group [get_clocks lf_clk] -group [get_clocks wb_clk] -group [get_clocks pixel_clk] -group [get_clocks byte_clk] -group [get_clocks tx_clk] -asynchronous
